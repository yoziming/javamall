    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          operateMan: [
            { required: true, message: '操作人[用户；系統；後台管理員]不能为空', trigger: 'blur' }
          ],
          createTime: [
            { required: true, message: '操作時間不能为空', trigger: 'blur' }
          ],
          orderStatus: [
            { required: true, message: '訂單狀態【0->待付款；1->待發貨；2->已發貨；3->已完成；4->已關閉；5->無效訂單】不能为空', trigger: 'blur' }
          ],
          note: [
