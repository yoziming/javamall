<template>
  <el-dialog
    ...
    <el-form-item label="該商品經過優惠後的分解金額" prop="realAmount">
      <el-input v-model="dataForm.realAmount" placeholder="該商品經過優惠後的分解金額"></el-input>
    </el-form-item>
    <el-form-item label="贈送積分" prop="giftIntegration">
      <el-input v-model="dataForm.giftIntegration" placeholder="贈送積分"></el-input>
    </el-form-item>
    <el-form-item label="贈送成長值" prop="giftGrowth">
      <el-input v-model="dataForm.giftGrowth" placeholder="贈送成長值"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          categoryId: [
            { required: true, message: '商品分類id不能为空', trigger: 'blur' }
          ],
          skuId: [
            { required: true, message: '商品sku編號不能为空', trigger: 'blur' }
          ],
          skuName: [
            { required: true, message: '商品sku名字不能为空', trigger: 'blur' }
          ],
          skuPic: [
            { required: true, message: '商品sku圖片不能为空', trigger: 'blur' }
          ],
          skuPrice: [
            { required: true, message: '商品sku價格不能为空', trigger: 'blur' }
          ],
          skuQuantity: [
            { required: true, message: '商品購買的數量不能为空', trigger: 'blur' }
          ],
          skuAttrsVals: [
            { required: true, message: '商品銷售屬性組合（JSON）不能为空', trigger: 'blur' }
          ],
          promotionAmount: [
            { required: true, message: '商品促銷分解金額不能为空', trigger: 'blur' }
          ],
          couponAmount: [
            { required: true, message: '優惠券優惠分解金額不能为空', trigger: 'blur' }
          ],
          integrationAmount: [
            { required: true, message: '積分優惠分解金額不能为空', trigger: 'blur' }
          ],
          realAmount: [
            { required: true, message: '該商品經過優惠後的分解金額不能为空', trigger: 'blur' }
          ],
          giftIntegration: [
            { required: true, message: '贈送積分不能为空', trigger: 'blur' }
          ],
          giftGrowth: [
            { required: true, message: '贈送成長值不能为空', trigger: 'blur' }
          ]
        }
      }
    },
