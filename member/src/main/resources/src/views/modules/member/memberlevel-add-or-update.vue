<template>
  <el-dialog
    ...
    <el-form-item label="等級需要的成長值" prop="growthPoint">
      <el-input v-model="dataForm.growthPoint" placeholder="等級需要的成長值"></el-input>
    </el-form-item>
    <el-form-item label="是否為默認等級[0->不是；1->是]" prop="defaultStatus">
      <el-input v-model="dataForm.defaultStatus" placeholder="是否為默認等級[0->不是；1->是]"></el-input>
    </el-form-item>
    <el-form-item label="免運費標準" prop="freeFreightPoint">
      <el-input v-model="dataForm.freeFreightPoint" placeholder="免運費標準"></el-input>
    </el-form-item>
    <el-form-item label="每次評價獲取的成長值" prop="commentGrowthPoint">
      <el-input v-model="dataForm.commentGrowthPoint" placeholder="每次評價獲取的成長值"></el-input>
    </el-form-item>
    <el-form-item label="是否有免郵特權" prop="priviledgeFreeFreight">
      <el-input v-model="dataForm.priviledgeFreeFreight" placeholder="是否有免郵特權"></el-input>
    </el-form-item>
    <el-form-item label="是否有會員價格特權" prop="priviledgeMemberPrice">
      <el-input v-model="dataForm.priviledgeMemberPrice" placeholder="是否有會員價格特權"></el-input>
    </el-form-item>
    <el-form-item label="是否有生日特權" prop="priviledgeBirthday">
      <el-input v-model="dataForm.priviledgeBirthday" placeholder="是否有生日特權"></el-input>
    </el-form-item>
  ...
  export default {
    data () {
      return {
        ...
        dataRule: {
          name: [
            { required: true, message: '等級名稱不能为空', trigger: 'blur' }
          ],
          growthPoint: [
            { required: true, message: '等級需要的成長值不能为空', trigger: 'blur' }
          ],
          defaultStatus: [
            { required: true, message: '是否為默認等級[0->不是；1->是]不能为空', trigger: 'blur' }
          ],
          freeFreightPoint: [
            { required: true, message: '免運費標準不能为空', trigger: 'blur' }
          ],
          commentGrowthPoint: [
            { required: true, message: '每次評價獲取的成長值不能为空', trigger: 'blur' }
          ],
          priviledgeFreeFreight: [
            { required: true, message: '是否有免郵特權不能为空', trigger: 'blur' }
          ],
          priviledgeMemberPrice: [
            { required: true, message: '是否有會員價格特權不能为空', trigger: 'blur' }
          ],
          priviledgeBirthday: [
            { required: true, message: '是否有生日特權不能为空', trigger: 'blur' }
          ],
          note: [
