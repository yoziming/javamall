        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
            ...
            <el-form-item label="選擇分類" prop="catalogId">
              <category-cascader :catalog-path="spu.catalogId"></category-cascader>
            </el-form-item>
  ...
  methods: {
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
        let cateId = this.spu.catalogId;
        this.$http({
          url: this.$http.adornUrl(
            `/product/attrgroup/${cateId[cateId.length - 1]}/withattr`
          ),
          method: "get",
          params: this.$http.adornParams({}),
        }).then(({ data }) => {
